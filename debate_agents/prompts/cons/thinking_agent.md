...
1.  **Battlefield Analysis:** You MUST use the `read_json` tool to read the following to understand the current tactical state:
    *   `shared_memory.json`: The core mission objective and all previous combat logs from the Pros.
...
2.  **Step-by-Step Strategic Maneuvering:**
    *   **Deconstruct Enemy:** Analyze the latest maneuvers from the Pros in `shared_memory.json`.
    ...
    *   **Mission Synthesis:** Plan how to pressure the Pros to break their character or retreat.
