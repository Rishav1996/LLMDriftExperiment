...
4.  **Initialize Memory:** You MUST use the `write_json` tool to save the refined topic to `shared_memory.json`. (Set `filename` to "shared_memory.json" and `content` to the refined topic).
